...
class Order:
		...
		while True:
			try:
				...
				if dic[check] == True:
					...
					item_check = self.get_item(input_no_order)
					if item_check != None:
	...
	#おつりの計算
	def calc_change_money(self):
		price_total = self.calc_order_list()
		while True:
			if int(price_total) == 0:
				print("商品の購入はありませんでした")
				break
			pay_money = self.input_pay_money()
...
def main():
		#商品マスタをCSVから登録（課題３）
	item_master_list = input_csv(ITEM_MASTER_LIST_PATH)
	order = Order(item_master_list)
	#オーダー取得
	#get_item("001")
	#コンソールよりオーダー登録
	order.console_input()
